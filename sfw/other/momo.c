...
void drawMyRectangle(int x,int y,int w,int h, uint32 color)
{
    ...
    mrc_drawLine(x+2,y,a-3,y,PIXEL888RED(color),PIXEL888GREEN(color),PIXEL888BLUE(color));//上
    mrc_drawLine(x+2,b-1,a-3,b-1,PIXEL888RED(color),PIXEL888GREEN(color),PIXEL888BLUE(color));//下

    mrc_drawLine(x,y+2,x,b-3,PIXEL888RED(color),PIXEL888GREEN(color),PIXEL888BLUE(color));//左
    mrc_drawLine(a-1,y+2,a-1,b-3,PIXEL888RED(color),PIXEL888GREEN(color),PIXEL888BLUE(color));//右
...
int GetMediumFontWH(int32 *w, int32 *h, uint16 font)
{
    ...
    ret = mrc_textWidthHeight("鼎", 0, font, &w1, &h1);
    if(w) *w=w1;
    if(h) *h=h1;
    return ret;
}
...
char *getPathPath(char* path)
{
    ...
    mrc_memcpy(name, path, len);//不用STRNCPY
